...
namespace Microsoft.Test.RenderingVerification.Filters
{
    ...
    public class NormalizedColorConverter: ExpandableObjectConverter
    {
        /// <summary>
        ...
        {
            if (destinationType == typeof(NormalizedColor))
            {
                return true;
            }
        ...
        {
            if (destinationType == typeof(string) && value is NormalizedColor)
            {
                return ((NormalizedColor)value).ToString();
            }
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, System.Globalization.CultureInfo culture, object value)
        {
            ...
            { 
                try
                {
                    NormalizedColor color = new NormalizedColor();
                    if (((string)value).Trim().ToLower() != NormalizedColor.Empty.ToString().ToLower())
                    { 
/*
...
*/
                        ...
                        color.Alpha = double.Parse(channels[0]);
                        color.Red= double.Parse(channels[1]);
                        color.Green = double.Parse(channels[2]);
                        color.Blue = double.Parse(channels[3]);
                    }
                    return color;
                }
                catch 
