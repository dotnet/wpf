...
public class DrtInterleavedPackage
{
    ...
    private int CreateInterleavedPackage()
    {
        ...
        Package package = OpenPackage("interleaved.zip", FileMode.Create, FileAccess.ReadWrite, FileShare.None);
    ...
    private int EditInterleavedPackage()
    {
        ...
        Package package = Package.Open("interleaved.zip", FileMode.Open, FileAccess.ReadWrite);
        ...
        Stream specStream = specPart.GetStream(FileMode.Open, FileAccess.ReadWrite);
    ...
    private Package OpenPackage(string file, FileMode packageMode, FileAccess packageAccess, System.IO.FileShare packageShare)
    {
        using (var fileStream = new FileStream(file, packageMode, packageAccess, packageShare))
        {
            return Package.Open(fileStream, packageMode, packageAccess);
        }
    }
    ...
    private bool CheckContentOfParts()
    {
        ...
        Package package = Package.Open("interleaved.zip", FileMode.Open, FileAccess.Read, FileShare.Read);
    ...
    private bool CheckRelationships()
    {
        ...
        Package package = Package.Open("interleaved.zip", FileMode.Open, FileAccess.Read);
        PackagePart poemPart = package.GetPart(new Uri(_poemUri, UriKind.Relative));
        PackagePart specPart = package.GetPart(new Uri(_specUri, UriKind.Relative));
