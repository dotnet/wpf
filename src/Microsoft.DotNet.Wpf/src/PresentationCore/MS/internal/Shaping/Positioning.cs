...
namespace MS.Internal.Shaping
{
                                      ...
                                      )
        {
            ...
            if (StaticGlyph < MobileGlyph)
                for (int i = StaticGlyph + 1; i < MobileGlyph; i++)
                    AdvanceInBetween += Advances[i];
            else
                for (int i = MobileGlyph + 1; i < StaticGlyph; i++)
                    AdvanceInBetween += Advances[i];
    ...
    internal struct DeviceTable
    {
        ...
        public int Value(FontTable Table, ushort PixelsPerEm)
        {
            ...
            if (PixelsPerEm < startSize || PixelsPerEm > endSize)
                return 0;
    ...
    internal struct PairPositioningSubtable
    {
        ...
        private struct PairSetTable
        {
            ...
            public int FindPairValue(FontTable Table, ushort Glyph)
            {
                ...
                {
                    if (PairValueGlyph(Table, i) == Glyph)
                        return i;
                }
                return -1;
            }
                ...
                case 2:
                    {
                        ...
                        if (firstClassIndex >= Format2Class1Count(Table))
                            return false; //this is invalid font;
                        ushort secondClassIndex = Format2Class2Table(Table).GetClass(Table, secondGlyphId);
                        if (secondClassIndex >= Format2Class2Count(Table))
                            return false; //this is invalid font;
    ...
    internal struct MarkToBasePositioningSubtable
    {
            ...
            )
        {
            ...
            if ((GlyphInfo.GlyphFlags[markGlyph] & (ushort)GlyphFlags.GlyphTypeMask) != (ushort)GlyphFlags.Mark)
                return false;
            ...
            if (markClass >= classCount)
                return false; //Invalid mark class
    ...
    internal struct MarkToMarkPositioningSubtable
    {
            ...
            )
        {
            ...
            //Lookup works with marks only
            if ((GlyphInfo.GlyphFlags[mark1Glyph] & (ushort)GlyphFlags.GlyphTypeMask) != (ushort)GlyphFlags.Mark)
                return false;

            int mark1CoverageIndex = Mark1Coverage(Table).GetGlyphIndex(Table, GlyphInfo.Glyphs[mark1Glyph]);
            if (mark1CoverageIndex == -1)
                return false;
