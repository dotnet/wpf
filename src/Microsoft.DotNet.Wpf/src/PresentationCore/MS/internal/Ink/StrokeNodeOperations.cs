...
namespace MS.Internal.Ink
{
    ...
    internal partial class StrokeNodeOperations
    {
        ...
        /// <returns></returns>
        internal virtual IEnumerable<ContourSegment> GetNonBezierContourSegments(in StrokeNodeData beginNode, in StrokeNodeData endNode)
        {
        ...
        private bool HitTestStrokeNodes(
            ContourSegment hitSegment, in StrokeNodeData beginNode, in StrokeNodeData endNode, ref StrokeFIndices result)
        {
            // First, find out if hitSegment intersects with either of the ink nodes
            bool isHit = false;
            for (int node = 0; node < 2; node++)
