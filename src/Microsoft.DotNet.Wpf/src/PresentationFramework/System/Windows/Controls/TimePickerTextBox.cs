...
{
    /// <summary>
    /// TimePickerTextBox is a specialized form of TextBox which displays custom visuals when its contents are empty
    /// </summary>
    [TemplatePart(Name = TimePickerTextBox.ElementContentName, Type = typeof(ContentControl))]
    public sealed partial class TimePickerTextBox : TextBox
    {
        #region Constants
        ...
        static TimePickerTextBox()
        {
            ...
            TextProperty.OverrideMetadata(typeof(TimePickerTextBox), new FrameworkPropertyMetadata(OnVisualStatePropertyChanged));
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="TimePickerTextBox"/> class.
        /// </summary>
        public TimePickerTextBox()
        {
            this.SetCurrentValue(WatermarkProperty, SR.TimePickerTextBox_DefaultWatermarkText);
            this.Loaded += OnLoaded;
            this.IsEnabledChanged += new DependencyPropertyChangedEventHandler(OnTimePickerTextBoxIsEnabledChanged);
        }
        #endregion
        ...
        internal static readonly DependencyProperty WatermarkProperty = DependencyProperty.Register(
            "Watermark", typeof(object), typeof(TimePickerTextBox), new PropertyMetadata(OnWatermarkPropertyChanged));
        ...
        private static T ExtractTemplatePart<T>(string partName, DependencyObject obj) where T : DependencyObject
        {
            Debug.Assert(
                obj == null || obj is T,
                string.Format(CultureInfo.InvariantCulture, SR.TimePickerTextBox_TemplatePartIsOfIncorrectType, partName, typeof(T).Name));
            return obj as T;
        }
        ...
        private void OnTimePickerTextBoxIsEnabledChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
        ...
        {
            TimePickerTextBox TimePickerTextBox = sender as TimePickerTextBox;
            Debug.Assert(TimePickerTextBox != null, "The source is not an instance of a TimePickerTextBox!");
            TimePickerTextBox.OnWatermarkChanged();
            TimePickerTextBox.UpdateVisualState();
        }
