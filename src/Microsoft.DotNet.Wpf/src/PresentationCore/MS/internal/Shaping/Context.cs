...
namespace MS.Internal.Shaping
{
    ...
    internal struct ClassChainingSubtable
    {
            ...
            )
        {
            ...
            if (Coverage(Table).GetGlyphIndex(Table, glyphId) < 0)
                return false;
            ...
            if (GlyphClass >= ClassSetCount(Table))
                return false; //!!! Bad font table

            SubClassSet subClassSet = ClassSet(Table, GlyphClass);
            if (subClassSet.IsNull)
                return false;   // There are no rules for this class
